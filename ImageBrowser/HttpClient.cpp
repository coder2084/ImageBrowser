...
const size_t HttpClient::NUM_THREADS = 10;
const int64_t HttpClient::MAX_IMG_SIZE = 120000;
...
size_t write_data(void *ptr, size_t size, size_t nmemb, void *stream) 
{
	...
	memcpy_s(d->buffer.get()+d->offset, HttpClient::MAX_IMG_SIZE, ptr, size * nmemb);
	d->offset += size * nmemb;
	...
	:mThreadTerminate(false)
{
	...
	for (auto& c : mCurls)
	{
		...
		c.buffer = std::make_unique<char[]>(MAX_IMG_SIZE);
		c.threadId = std::make_unique<std::thread>([&, this] { this->Update(c); });
	}
}
...
void HttpClient::Update(DownloadData& downloadData)
{
	while (!mThreadTerminate)
	{
		...
		{
			request->GetCallback()->HttpSuccess(downloadData.buffer.get(), downloadData.offset);
		}
